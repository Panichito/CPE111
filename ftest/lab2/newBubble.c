...
void swapStrings(char** array, long int index1, long int index2);
...
int main(int argc, char* argv[])
{
	...
	data = calloc(count, sizeof(char*)); // Here !
	if (data == NULL)
	{
...
void sortArray(char** array, long int count)
{
	...
	while (swapCount > 0)
	{
		...
		for (i = 0; i < count-1; i++)
		{
			...
			if (strcmp(array[i], array[i+1]) > 0)   // Here !
			{
				swapStrings(array, i, i+1);   // Here !
				swapCount++;
			}
		}    
	}
