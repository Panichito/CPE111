 ...
 *      add new function listItemSwap , swap item position in list
 *      add new function listSwapData , swap the pointer to data of a item in list
 */
...
* Arguments:
*   linkedList  -   list we want to swap from
...
* Returns NULL if the list is empty or the list pointer is NULL
*/
void *listItemSwap(LIST_HANDLE list, int position1, int position2)
    {
    void *data = NULL;
    LIST_T *linkedList = (LIST_T *) list;
    ...
    if ((linkedList != NULL) && (linkedList->head != NULL))
        {
        if (position1 >= 0 && position2 <= listSize(linkedList))
            {
            LISTITEM_T *itemTempBefore1 = NULL;
            LISTITEM_T *itemTemp1 = NULL;
            LISTITEM_T *itemTempBefore2 = NULL;
            LISTITEM_T *itemTemp2 = NULL;

            LISTITEM_T *itemTempNext = NULL;
            ...
            for (int i = 0; i <= position2; i++)
                {
                if (i == position1 - 1 && itemTemp1 == NULL)
                    {
                    itemTempBefore1 = pThis;
                    itemTemp1 = pThis->pNext;
                    //printf("get item 1 at %d , %p , %p\n", i, itemTempBefore1->pNext, itemTemp1);
                    }
                else if (i == position2 - 1 && itemTemp2 == NULL)
                    {
                    itemTempBefore2 = pThis;
                    itemTemp2 = pThis->pNext;
                    //printf("get item 2 at %d , %p , %p\n", i, itemTempBefore2->pNext, itemTemp2);
                    }
                pThis = pThis->pNext;
                }
            if (itemTempBefore1 != NULL)
                {
                itemTempBefore1->pNext = itemTemp2;
                }
            else
                {
                linkedList->head = itemTemp2;
                }
            itemTempBefore2->pNext = itemTemp1;
            itemTempNext = itemTemp1->pNext;
            itemTemp1->pNext = itemTemp2->pNext;
            itemTemp2->pNext = itemTempNext;
            if (itemTemp1->pNext == NULL)
                {
                linkedList->tail = itemTemp1;
                }
            data = (void *) itemTemp2;
            }
        }
...
void *listItemMove(LIST_HANDLE list, int position1, int position2)
    ...
    if (position1 > list_size)
        position1 = list_size;
    if (position2 > list_size)
        position2 = list_size;
    ...
    if ((linkedList != NULL) && (linkedList->head != NULL))
        {
        LISTITEM_T *itemTempBeforeFrom = NULL;
        LISTITEM_T *itemTempFrom = NULL;
        LISTITEM_T *itemTempBeforeTo = NULL;
        LISTITEM_T *itemTempTo = NULL;
        ...
        for (int i = 0; i <= maxPosition; i++)
            {
            if (i == position1 - 1 && itemTempFrom == NULL)
                {
                itemTempBeforeFrom = pThis;
                itemTempFrom = pThis->pNext;
                }
            else if (i == position2 - 1 && itemTempTo == NULL)
                {
                itemTempBeforeTo = pThis;
                itemTempTo = pThis->pNext;
                }
            pThis = pThis->pNext;
            }


        if (itemTempBeforeFrom != NULL)
            {
            itemTempBeforeFrom->pNext = itemTempFrom->pNext;
            if (itemTempBeforeFrom->pNext == NULL)
                {
                linkedList->tail = itemTempBeforeFrom;
                }
            }
        else
            linkedList->head = itemTempFrom->pNext;



        if (itemTempBeforeTo != NULL)
            {
            if (itemTempTo->pNext == NULL && (position1 < position2))
                {
                linkedList->tail = itemTempFrom;
                itemTempTo->pNext = itemTempFrom;
                itemTempFrom->pNext = NULL;
                }
            else if (itemTempTo->pNext != NULL && (position1 < position2))
                {
                itemTempFrom->pNext = itemTempTo->pNext;
                itemTempTo->pNext = itemTempFrom;
                }
            else
                {
                itemTempBeforeTo->pNext = itemTempFrom;
                itemTempFrom->pNext = itemTempTo;
                }
            }
        else
            {
            linkedList->head = itemTempFrom;
            itemTempFrom->pNext = itemTempTo;
            }
    
        
        data = (void *) itemTempFrom->pData;
        }
    return data;
    }
...
void *listSwapData(LIST_HANDLE list, void *oldData, void *newData)
    ...
    if ((linkedList != NULL) && (linkedList->head != NULL))
        {
        LISTITEM_T *pThis = linkedList->head;
        for (int i = 0; i <= listSize(linkedList); i++)
            {
            if (pThis->pData == oldData)
                {
                pThis->pData = newData;
                data = pThis->pData;
                break;
