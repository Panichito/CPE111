...
void printPathAndWeight(int index1, int index2)
{
	...
	while(vertices[currentIndex].parentIndex != -1) 
	{
		currentIndex = vertices[currentIndex].parentIndex;  /* store parent of last index first for easy to print as Aj Sally format*/
		path[order++] = vertices[currentIndex].keyValue;
	}
	printf("Min total weight of shortest path: %d\n",vertices[index2].dValue);
	for(int i = MAXVERT-1; i >= 0; --i) 
	{
		if(path[i] == 0) 
			continue;
		printf("'%d' => ",path[i]);
	}
	printf("'%d'\n",vertices[index2].keyValue);  /* last element no longer => */
}
...
void printShortestPath(int index1, int index2)
{
	...
	/** ADD MY CODE FOR DIJKSTR'S ALGORITHM HERE **/
	while(queueMinSize() != 0) 
	{
		currentVtx = dequeueMin();
		currentVtx->color = BLACK;
		/* printf("%d  %d\n",currentVtx->keyValue,currentVtx->myIndex); */  /* BIG DEBUG */
		currentIndex = findVertex(currentVtx->keyValue);
		for(int i = 0; i < MAXVERT; ++i) 
		{
			if(edges[currentIndex][i] != 0) {  /* adjacent to current */
				weight = edges[currentIndex][i];
				if(currentVtx->dValue + weight < vertices[i].dValue) 
				{
					vertices[i].dValue = currentVtx->dValue + weight;  /* sum */
					vertices[i].parentIndex = currentVtx->myIndex;
				}
			}
		}
	}
